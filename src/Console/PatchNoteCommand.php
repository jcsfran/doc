...
class PatchNoteCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'docs:patch {name} {--routes=1 : The number of routes modified in this update }';
    ...
    public function handle()
    {
        ...
        return $this->info('Created Patch Note: ' . $fileName);
    }
    ...
    private function verifyIfFolderExists(): void
    {
        if (!is_dir(config('documentation.patch_notes_path'))) {
            mkdir(config('documentation.patch_notes_path'), 755, true);
        }
    }
}
